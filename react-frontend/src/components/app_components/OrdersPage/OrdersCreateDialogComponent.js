...
const OrdersCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            product: _entity?.product,qty: _entity?.qty,price: _entity?.price,
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber tooltip="Use only numbers" id="qty" className="w-full mb-3" value={_entity?.qty} onChange={(e) => setValByKey("qty", e.value)}  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["qty"]) ? (
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber tooltip="Use only numbers" id="price" className="w-full mb-3" mode="currency" currency="MYR" locale="en-US" value={_entity?.price} onValueChange={(e) => setValByKey("price", e.value)}  />
            </span>
            <small className="p-error">
