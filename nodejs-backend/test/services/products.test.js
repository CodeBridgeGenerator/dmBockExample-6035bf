...
describe("products service", async () => {
  ...
  const categoryCreated = await app.service("category").Model.create({"name":"new value","sku":"new value","category":"parentObjectId"});
  ...
  describe("#create", () => {
    const options = {"name":"new value","sku":"new value","category":`${categoryCreated._id}`};
...
assert.strictEqual(productCreated.category.toString(), options.category.toString());
  ...
  describe("#update", () => {
    const options = {"name":"updated value","sku":"updated value","category":`${categoryCreated._id}`};
